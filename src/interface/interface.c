...
enum index_margem{
    TITLE, RANKING, NAME, VICTORIES, DEFEATS, DRAWS 
};

int size_table = 70, size_space = 14, margem_start = 25, margem_const = 7;
string_center margem[7] = {0};
register int index = 0;
ranking *rank = NULL;


margem[TITLE] = CenterString(message_interface[INTERFACE_RANKING_TITLE], size_table);
margem[RANKING] = CenterString(message_interface[INTERFACE_PRINT_RANKING_RANKING], size_space);
margem[NAME] = CenterString(message_interface[INTERFACE_PRINT_RANKING_NAMES], size_space);
margem[VICTORIES] = CenterString(message_interface[INTERFACE_PRINT_RANKING_VICTORIES], size_space);
margem[DEFEATS] = CenterString(message_interface[INTERFACE_PRINT_RANKING_DEFEATS], size_space);
margem[DRAWS] = CenterString(message_interface[INTERFACE_PRINT_RANKING_DRAW], size_space);


printchar_menu(margem_start, ' ', '_', size_table, ' ');
printchar_menu(margem_start, '|' , ' ', size_table, '|');
printf("%*c%*s%*c\n", margem_start, '|', margem[TITLE].start, message_interface[INTERFACE_RANKING_TITLE], margem[TITLE].end, '|');
printchar_menu(margem_start, '|' , '_', size_table, '|');


printf("%*c%*s%*c%*s%*c%*s%*c%*s%*c%*s%*c\n", margem_start, '|', 
                                              margem[RANKING].start, message_interface[INTERFACE_PRINT_RANKING_RANKING], 
                                              margem[RANKING].end, '|', 
                                              margem[NAME].start, message_interface[INTERFACE_PRINT_RANKING_NAMES], 
                                              margem[NAME].end, '|', 
                                              margem[VICTORIES].start, message_interface[INTERFACE_PRINT_RANKING_VICTORIES], 
                                              margem[VICTORIES].end, '|',
                                              margem[DEFEATS].start, message_interface[INTERFACE_PRINT_RANKING_DEFEATS], 
                                              margem[DEFEATS].end, '|', 
                                              margem[DRAWS].start, message_interface[INTERFACE_PRINT_RANKING_DRAW],
                                              margem[DRAWS].end, '|');


for(index = 0; index < CountUser; index++){


(*RankUser)++;
rank = &UserRanking[index];
margem[NAME] = CenterString(rank->name, size_space);


printf("%*c%*d%*c%*s%*c%*d%*c%*d%*c%*d%*c\n", margem_start, '|', 
                                             margem_const, *RankUser, 
                                             margem_const, '|', 
                                             margem[NAME].start, rank->name, 
                                             margem[NAME].end, '|', 
                                             margem_const, rank->vitorias, 
                                             margem_const, '|', 
                                             margem_const, rank->derrotas, 
                                             margem_const, '|', 
                                             margem_const, rank->empate,
                                             margem_const, '|');
...
printf("%*c", margem_start,'|');
print_table(size_space);
print_table(size_space);
print_table(size_space);
print_table(size_space);
print_table(size_space);
...
const char *mensagem[] = {message_interface[INTERFACE_PLACAR_HEADER_CONFRONTO],
                    message_interface[INTERFACE_PLACAR_HEADER_PARTIDA]};

char *control_string[] = {"#s#s#s",
                          "#s#i#i",
                          "#s#i#i",
                          "#s#i#i",
                          "#s#i#i"};
...
config_cells cells = {0};
box_config box = {0};


//config cells
cells.cells_margin = 20;
cells.cells_size = 16;
cells.cells_count = 3;
...
//config chat_box
box.box_margin = 20;
box.box_horizontal_size = 48;
box.box_vertical_size = 2;
box.args_count = 1;


chat_box(box, mensagem[flag]);

cells.control_string = control_string[COLUMN_ONE];
print_cells(cells, message_interface[INTERFACE_PLACAR_PLAYERS], 
                    placar_da_partida->P1.player, 
                    placar_da_partida->P2.player);


cells.control_string = control_string[COLUMN_TWO];
print_cells(cells, message_interface[INTERFACE_PLACAR_VITORIA],
                    placar_da_partida->P1.vitorias,
                    placar_da_partida->P2.vitorias);


cells.control_string = control_string[COLUMN_THREE];
print_cells(cells, message_interface[INTERFACE_PLACAR_DERROTAS],
                    placar_da_partida->P1.derrotas,
                    placar_da_partida->P2.derrotas);


cells.control_string = control_string[COLUMN_FOUR];
print_cells(cells, message_interface[INTERFACE_PLACAR_EMPATES],
                    placar_da_partida->empate,
                    placar_da_partida->empate);


cells.control_string = control_string[COLUMN_FIVE];
print_cells(cells, message_interface[INTERFACE_PLACAR_PARTIDAS],
                    placar_da_partida->PartidasJogadas,
                    placar_da_partida->PartidasJogadas);
